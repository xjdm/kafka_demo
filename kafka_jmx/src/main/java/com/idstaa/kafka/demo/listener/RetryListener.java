...
import com.idstaa.kafka.demo.entity.RetryRecord;
...
public class RetryListener {
    ...
    // public void consume(List<ConsumerRecord<String, String>> list) { // for(ConsumerRecord<String, String> record : list){
    public void consume(ConsumerRecord<String, String> record) {
        System.out.println("需要重试的消息：" + record);
        RetryRecord retryRecord = JSON.parseObject(record.value(), RetryRecord.class); /**
         * 防止待重试消息太多撑爆redis,可以将待重试消息按下一次重试时间分开存储放到不 同介质
         * 例如下一次重试时间在半小时以后的消息储存到mysql,并定时从mysql读取即将重试 的消息储储存到redis
         */
        // 通过redis的zset进行时间排序
    ...
    public void retryFromRedis() {
        ...
        for (ZSetOperations.TypedTuple<Object> tuple : typedTuples) {
            ...
            ProducerRecord recordReal = new ProducerRecord(
                    bizTopic, record.partition(), record.timestamp(), record.key(), record.value(), record.headers());
